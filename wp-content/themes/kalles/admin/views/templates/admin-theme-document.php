<?php if ( ! defined( 'ABSPATH' )  ) { die; } // Cannot access directly. ?>
<div class="row">
	<div class="theme-document-item jus_between">
		<div class="theme-document-content w__full">
			<a href="<?php echo esc_url( 'https://kalleswp-docs.the4.co/' ); ?>" target="_blank">
				<div class="theme-document-icon">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><path d="M 9 4 C 7.355469 4 6 5.355469 6 7 L 6 25 C 6 26.644531 7.355469 28 9 28 L 26 28 L 26 4 Z M 9 6 L 24 6 L 24 22 L 9 22 C 8.648438 22 8.316406 22.074219 8 22.1875 L 8 7 C 8 6.433594 8.433594 6 9 6 Z M 11 9 L 11 11 L 22 11 L 22 9 Z M 9 24 L 24 24 L 24 26 L 9 26 C 8.433594 26 8 25.566406 8 25 C 8 24.433594 8.433594 24 9 24 Z"/></svg>
				</div>
				<div class="theme-document-body">
					<p class="theme-document_title color__title"><?php esc_html_e('Document', 'kalles'); ?></p>
					<p class="theme-document_info color__text"><?php esc_html_e('check theme document', 'kalles'); ?></p>
				</div>
			</a>
		</div>
	</div>
	<div class="theme-document-item">
		<div class="theme-document-content w__full">
			<a href="<?php echo esc_url( 'https://www.youtube.com/channel/UCvOVrojNhNbGnT2FnfHZ_BQ' ); ?>" target="_blank">
				<div class="theme-document-icon">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><path d="M 16 6 C 12.234375 6 8.90625 6.390625 6.875 6.6875 C 5.195313 6.933594 3.839844 8.199219 3.53125 9.875 C 3.269531 11.300781 3 13.386719 3 16 C 3 18.613281 3.269531 20.699219 3.53125 22.125 C 3.839844 23.800781 5.195313 25.070313 6.875 25.3125 C 8.914063 25.609375 12.253906 26 16 26 C 19.746094 26 23.085938 25.609375 25.125 25.3125 C 26.804688 25.070313 28.160156 23.800781 28.46875 22.125 C 28.730469 20.695313 29 18.605469 29 16 C 29 13.394531 28.734375 11.304688 28.46875 9.875 C 28.160156 8.199219 26.804688 6.933594 25.125 6.6875 C 23.09375 6.390625 19.765625 6 16 6 Z M 16 8 C 19.632813 8 22.878906 8.371094 24.84375 8.65625 C 25.6875 8.78125 26.347656 9.417969 26.5 10.25 C 26.742188 11.570313 27 13.527344 27 16 C 27 18.46875 26.742188 20.429688 26.5 21.75 C 26.347656 22.582031 25.691406 23.222656 24.84375 23.34375 C 22.871094 23.628906 19.609375 24 16 24 C 12.390625 24 9.125 23.628906 7.15625 23.34375 C 6.3125 23.222656 5.652344 22.582031 5.5 21.75 C 5.257813 20.429688 5 18.476563 5 16 C 5 13.519531 5.257813 11.570313 5.5 10.25 C 5.652344 9.417969 6.308594 8.78125 7.15625 8.65625 C 9.117188 8.371094 12.367188 8 16 8 Z M 13 10.28125 L 13 21.71875 L 14.5 20.875 L 21.5 16.875 L 23 16 L 21.5 15.125 L 14.5 11.125 Z M 15 13.71875 L 18.96875 16 L 15 18.28125 Z"/></svg>
					</svg>
				</div>
				<div class="theme-document-body">
					<p class="theme-document_title color__title"><?php esc_html_e('Videos tutorial', 'kalles'); ?></p>
					<p class="theme-document_info color__text"><?php esc_html_e('check lasted update version', 'kalles'); ?></p>
				</div>
			</a>
		</div>
	</div>
	<div class="theme-document-item jus_between">
		<div class="theme-document-content w__full">
			<a href="<?php echo esc_url( 'https://support.the4.co/login' ); ?>" target="_blank">
				<div class="theme-document-icon">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><path d="M 16 4 C 11.671875 4 8 7.054688 8 11 L 8 14 L 14 14 L 14 11 C 14 10.851563 14.089844 10.636719 14.4375 10.40625 C 14.785156 10.175781 15.351563 10 16 10 C 16.652344 10 17.21875 10.175781 17.5625 10.40625 C 17.90625 10.636719 18 10.835938 18 11 C 18 11.578125 17.8125 11.980469 17.4375 12.4375 C 17.0625 12.894531 16.480469 13.367188 15.84375 13.90625 C 14.566406 14.988281 13 16.496094 13 19 L 13 20 L 19 20 L 19 19 C 19 18.660156 19.125 18.421875 19.5 18.03125 C 19.875 17.640625 20.496094 17.179688 21.15625 16.625 C 22.476563 15.515625 24 13.816406 24 11 C 24 7.089844 20.335938 4 16 4 Z M 16 6 C 19.394531 6 22 8.367188 22 11 C 22 13.144531 21.023438 14.101563 19.84375 15.09375 C 19.253906 15.589844 18.625 16.074219 18.0625 16.65625 C 17.714844 17.015625 17.445313 17.484375 17.25 18 L 15.3125 18 C 15.625 16.988281 16.234375 16.21875 17.15625 15.4375 C 17.769531 14.921875 18.4375 14.382813 19 13.6875 C 19.5625 12.992188 20 12.082031 20 11 C 20 10.039063 19.429688 9.242188 18.6875 8.75 C 17.945313 8.257813 17.003906 8 16 8 C 14.992188 8 14.050781 8.257813 13.3125 8.75 C 12.574219 9.242188 12 10.042969 12 11 L 12 12 L 10 12 L 10 11 C 10 8.316406 12.597656 6 16 6 Z M 13 22 L 13 28 L 19 28 L 19 22 Z M 15 24 L 17 24 L 17 26 L 15 26 Z"/></svg>
				</div>
				<div class="theme-document-body">
					<p class="theme-document_title color__title"><?php esc_html_e('Support system', 'kalles'); ?></p>
				<p class="theme-document_info color__text"><?php esc_html_e('Submit a ticket', 'kalles'); ?></p>
				</div>
			</a>
		</div>
	</div>
	<div class="theme-document-item">
		<div class="theme-document-content w__full">
			<a href="<?php echo esc_url( 'https://kalleswp-docs.the4.co/changelog' ); ?>" target="_blank">
				<div class="theme-document-icon">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><path d="M 16 4 C 10.886719 4 6.617188 7.160156 4.875 11.625 L 6.71875 12.375 C 8.175781 8.640625 11.710938 6 16 6 C 19.242188 6 22.132813 7.589844 23.9375 10 L 20 10 L 20 12 L 27 12 L 27 5 L 25 5 L 25 8.09375 C 22.808594 5.582031 19.570313 4 16 4 Z M 25.28125 19.625 C 23.824219 23.359375 20.289063 26 16 26 C 12.722656 26 9.84375 24.386719 8.03125 22 L 12 22 L 12 20 L 5 20 L 5 27 L 7 27 L 7 23.90625 C 9.1875 26.386719 12.394531 28 16 28 C 21.113281 28 25.382813 24.839844 27.125 20.375 Z"/></svg>
				</div>
				<div class="theme-document-body">
					<p class="theme-document_title color__title"><?php esc_html_e('Changelog', 'kalles'); ?></p>
					<p class="theme-document_info color__text"><?php esc_html_e('check lasted update version', 'kalles'); ?></p>
				</div>
			</a>
		</div>
	</div>
</div>